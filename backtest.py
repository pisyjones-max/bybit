...
функцию (strategy.generate_signal_from_series) и ТУ ЖЕ САМУЮ логику
выхода (risk._check_exit), что и live-движок (engine.py вызывает их же).
Это не косметика — если бэктест и live считают сигнал по-разному,
бэктест ничего не говорит о том, что реально будет торговаться, и все
выводы из него бесполезны.

Также бэктест повторяет ограничение live-движка на размер скользящего
окна (MAX_POINTS баров) — индикаторы считаются не по всей истории, а по
последним 200 точкам (deque maxlen=200). Без этого бэктест был бы
"умнее" живого бота и давал бы завышенные результаты.

Объёмный фильтр: с момента перехода live-движка на OHLCV-свечи (get_kline
вместо тикера с volume24h) live и бэктест используют для vol_ratio ОДНО
И ТО ЖЕ — объём завершившегося бара. Ранее здесь было известное
расхождение (live брал скользящий 24-часовой volume24h тикера, а
бэктест — объём конкретной свечи, то есть разные по смыслу величины);
после рефакторинга оно устранено на уровне архитектуры, а не подгонкой
цифр.
...
from strategy import generate_signal_from_series, calc_rsi, RSI_PERIOD, MAX_POINTS, EMA_SLOW
from risk import _check_exit
...
def run_backtest(df: pd.DataFrame, cfg: BtConfig) -> dict:
    ...
    for i in range(len(prices)):
        ...
        if len(price_win) >= EMA_SLOW + 2:  # зеркалит условие в generate_signal_from_series
